...
while True:
	...
	if bg is not None:
		...
		ROI = frame[50:300,210:430]
		cv2.rectangle(frame,(210-2,50-2),(430+2,300+2),color_fingers,1)
		grayROI = cv2.cvtColor(ROI,cv2.COLOR_BGR2GRAY)

		# Región de interés del fondo de la imagen
		bgROI = bg[50:300,210:430]
		...
		for cnt in cnts:
			...
			if defects is not None:
				...
				byte_fingers = str(fingers).encode()
				print(byte_fingers)
				ser.write(byte_fingers)
	...
	# Guardar fondo de la imagen al presionar I
	if k == ord('i'):
		bg = cv2.cvtColor(frameAux,cv2.COLOR_BGR2GRAY)
	
	# Terminar ciclo al precionar Escape
	if k == 27:
		break
